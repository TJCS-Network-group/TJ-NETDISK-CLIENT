...
class NetDisk(QtWidgets.QMainWindow, Ui_NetDiskWindow):
    ...
    def __init__(self):
        ...
        upload_list = self.user_config['upload_list']
        for upload_index,upload_item in upload_list.items():
            print(upload_item)
            ...
            if upload_item['type'] == 'file':
                ...
                pid = upload_item['pid']
                fsize = upload_item['fsize']
                newThread = UploadFileThread()
                newThread.setOption(upload_index)
                ...
                self.listWidget.addItem(item)
                self.listWidget.setItemWidget(item, widget)
                dirname = self.user_config['upload_list'][upload_index]['dirname']
                haveUpload = self.user_config['upload_list'][upload_index]['haveUpload']
                widget.setFileNameText(dirname)
                widget.setProgressValue(haveUpload)

                # self.listWidget.itemWidget(item)
                self.upload_item_list[upload_index] = item

        download_list = self.user_config['download_list']
        for download_index,download_item in download_list.items():
            if download_item['type'] == 0:
                newThread = DownloadFileThread()
                newThread.setOption(download_index)
                ...
                widget.setFileNameText(download_item['filename'])
                widget.setFSize(download_item['fsize'])
                widget.setProgressValue(download_item['index']*FRAG_SIZE)
                widget.start()
                ...
                self.listWidget_2.addItem(item)
                self.listWidget_2.setItemWidget(item, widget)
                widget.setFileNameText(download_item['dirname'])
                widget.setFSize(download_item["totalSize"])
                task_count = download_item['download_task_count']
                now_value = download_item["haveDownload"]+download_item['file_download_list'][task_count]['index']*FRAG_SIZE
                widget.setProgressValue(now_value)
                widget.start()
                self.download_item_list[download_index] = item
    ...
    def downloadFunc(self):
        ...
        if dirPath != '':
            if downloadRow['type'] == 0:
                ...
                download_index = str(self.user_config['download_index'])
                ...
                download_index = str(self.user_config['download_index'])
                self.user_config['download_list'][download_index] = {"type": "dir", "save_path": dirPath, "haveDownload":0,
                                                                     "did": downloadRow['id'], "file_download_list": None,
                                                                     "isPause": False, "totalSize": 0, "download_task_count":0,
                                                                     "dirname": downloadRow['name']}
                self.user_config['download_index'] += 1
    ...
    def uploadDirFunc(self):
        ...
        if dirpath != '':
            ...
            upload_index = str(self.user_config['upload_index'])
            pid = self.getPid()
            self.user_config['upload_list'][upload_index] = {"type": "dir", "dirname": dirname, "dirpath": dirpath,
                                                             ...
                                                             "haveUpload": 0, "upload_task_count": 0}
