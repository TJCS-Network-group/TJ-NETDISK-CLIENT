...
class DownloadFileThread(QThread):
    ...
    def setOption(self, download_index: str):
        # , save_path: str, filename: str, fdid: int, fsize: int
        print("set option")
        self.client = client()
        ...
        self.save_path = self.user_config['download_list'][download_index]['save_path']
        self.filename = self.user_config['download_list'][download_index]['filename']
        self.fdid = self.user_config['download_list'][download_index]['fdid']
        self.fsize = self.user_config['download_list'][download_index]['fsize']
        self.index = self.user_config['download_list'][download_index]['index']
    ...
    def downloadFile(self, fdid, save_path, filename):
        ...
        if os.path.exists(new_path):
            ans = self.client.get_md5code(fdid)
            file_md5 = ans['data']['MD5']
            new_name = filename
            hasMD5 = False
            while True:
                test_md5 = hashlib.md5(open(os.path.join(save_path, new_name), "rb").read()).hexdigest()
        ...
        while self.index < max_index:
            if self.isPause:
                time.sleep(0.1)
                continue
            self.download_frag(fdid, frag_dir_path, self.index)
            self.trigger.emit(self.download_index, str(self.index))
            self.index += 1
            self.user_config['download_list'][self.download_index]['index'] = self.index
...
class DownloadDirThread(QThread):
    ...
    def setOption(self, download_index: str):
        ...
        self.tmp_path = self.user_config['tmp_path']
        self.download_index = download_index
        self.did = self.user_config['download_list'][self.download_index]['did']
        self.save_path = self.user_config['download_list'][self.download_index]['save_path']
        self.haveDownload = self.user_config['download_list'][self.download_index]['haveDownload']
        self.totalSize = self.user_config['download_list'][self.download_index]['totalSize']
        self.file_download_list = self.user_config['download_list'][self.download_index]['file_download_list']
        self.download_task_count = self.user_config['download_list'][self.download_index]['download_task_count']
        self.trigger.connect(self.triggerRecv)
    ...
    def run(self) -> None:
        try:
            if self.file_download_list is None:
                self.create_all_dir()

            # print("create dir")
            self.trigger.emit(self.download_index, "start")

            while self.download_task_count < len(self.file_download_list):
                file_download = self.file_download_list[self.download_task_count]
                self.downloadFile(file_download)
                self.haveDownload += file_download['fsize']
                self.download_task_count += 1
                self.user_config['download_list'][self.download_index]['download_task_count'] = self.download_task_count
                self.user_config['download_list'][self.download_index]['haveDownload'] = self.haveDownload
                config.set_user_config(self.current_user, self.user_config)
        ...
        for tree in tree_list:
            if tree['type'] == 0:
                ...
                if os.path.exists(new_path):
                    ...
                    hasMD5 = False
                    while True:
                        test_md5 = hashlib.md5(open(os.path.join(path, new_name), "rb").read()).hexdigest()
                        if test_md5 == file_md5:
                            hasMD5 = True
                            break
                        new_name_vec = new_name.split('.')
