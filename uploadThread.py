...
class UploadFileThread(QThread):
    ...
    def setOption(self, upload_index: str):
        print("set option")
        ...
        self.pid = self.user_config['upload_list'][upload_index]['pid']
        self.filepath = self.user_config['upload_list'][upload_index]['filepath']
...
class UploadDirThread(QThread):
    ...
    def setOption(self, upload_index: str):
        ...
        self.user_config = config.get_user_config(self.current_user)
        self.client.headers['Cookie'] = self.user_config['Cookie']
        self.dirpath = self.user_config['upload_list'][self.upload_index]['dirpath']
        self.pid = self.user_config['upload_list'][self.upload_index]['pid']
        self.file_upload_list = self.user_config['upload_list'][self.upload_index]['file_upload_list']
        self.upload_task_count = self.user_config['upload_list'][self.upload_index]['upload_task_count']
        self.haveUpload = self.user_config['upload_list'][self.upload_index]['haveUpload']
        self.trigger.connect(self.triggerRecv)
    ...
    def run(self) -> None:
        try:
            if self.file_upload_list is None:
                self.create_all_dir()
            print("create dir")
            self.trigger.emit(self.upload_index, "start")
            print("emit start",self.file_upload_list)
            while self.upload_task_count < len(self.file_upload_list):
                file_upload = self.file_upload_list[self.upload_task_count]
                self.uploadFile(file_upload)
                self.haveUpload += file_upload['fsize']
                self.upload_task_count += 1
                self.user_config['upload_list'][self.upload_index]['upload_task_count'] = self.upload_task_count
                self.user_config['upload_list'][self.upload_index]['haveUpload'] = self.haveUpload
